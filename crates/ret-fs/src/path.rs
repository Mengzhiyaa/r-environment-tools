...
/// - Windows Registry entries (e.g., `C:\Program Files\R\R-4.4.1\`)
/// - Tool output such as `conda info --json`
...
///    and hash generation (for example, resolved installation cache entries)
///
...
///    - Tool metadata files and command output
///    - Environment variables
///
/// 3. **Storing/Displaying Paths**: Ensures paths are in a canonical form for storage
///    and display (for example, `RInstallation.executable` and `RInstallation.home`)
///
/// # Important Notes
///
...
///
/// # Related
/// - `resolve_symlink()` - Filtered version for Python, Conda, and R executables
...
///
/// # Filtering
/// This function only resolves symlinks for Python, Conda, and R related executables:
/// - Files starting with `python` or `conda` (e.g., `python3.12`, `conda`)
/// - Files named `R` or starting with `Rscript` (e.g., `Rscript`)
/// - Excludes files ending with `-config` or `-build` (e.g., `python3-config`)
///
/// # Use Cases
/// - Identifying the actual executable behind symbolic links
/// - Homebrew symlinks: `/opt/homebrew/bin/python3.12` or `/opt/homebrew/bin/R` → actual Cellar path
/// - Tracking all symlink variants of an installation
///
/// # Related
...
pub fn resolve_symlink<T: AsRef<Path>>(exe: &T) -> Option<PathBuf> {
    ...
    // We support resolving Python, Conda, and R symlinks.
    let name_lower = name.to_lowercase();
    if !name_lower.starts_with("python")
        && !name_lower.starts_with("conda")
        && name_lower != "r"
        && !name_lower.starts_with("rscript")
    {
        return None;
    }
