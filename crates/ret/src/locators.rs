...
pub fn identify_r_installation_using_locators(
    ...
    let discovered = locators.iter().find_map(|locator| locator.try_from(env));
    ...
    if let Some(installation) = locators
        .iter()
        .find_map(|locator| locator.try_from(&resolved_env))
    {
        let installation = merge_resolved_installation(installation, &resolved);
...
fn merge_resolved_installation(
    ...
    symlinks.push(resolved.executable.clone());
